...
impl BottomBar {
    pub fn new(rect: Rectangle, current_page: usize, pages_count: usize, name: &str) -> BottomBar {
        ...
        let server_label = Label::new(server_rect, name.to_string(), Align::Center)
            .event(Some(Event::ToggleNear(ViewId::OpdsServerMenu, server_rect)));

        children.push(Box::new(server_label) as Box<dyn View>);
    ...
    pub fn update_server_label(&mut self, name: &str, rq: &mut RenderQueue) {
        let server_label = self.children[1].as_mut().downcast_mut::<Label>().unwrap();
        server_label.update(name, rq);
    }
